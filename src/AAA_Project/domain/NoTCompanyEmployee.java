...
public class NoTCompanyEmployee implements PersonInterface{
    @Override
    ...
    public NoTCompanyEmployee (String name, String email, int numberCitizen, int tin, int phoneNumber, Location location, String role) {
    ...
    public NoTCompanyEmployee(String name, String email, int numberCitizen, int tin, int phoneNumber, String role) {
        if(validateInputs(numberCitizen) && validateInputs(tin) && validateInputs(phoneNumber)){
        }else{
            throw new IllegalArgumentException("Arguments are not valid, please try again.");
        }
